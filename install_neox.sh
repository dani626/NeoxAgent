...
# ═══════════════════════════════════════════════════════════════════
#  neoxagent All-in-One Installer v0.5
#  Installs Rust, compiles NeoxAgent (with native TLS/SSL),
#  compiles hev-socks5-tproxy, configures ZRAM, systemd, kernel, firewall.
#
#  Usage:
...
print_header() {
    ...
    echo "  ╚═╝  ╚═══╝╚══════╝ ╚═════╝ ╚═╝  ╚═╝"
    echo -e "${NC}"
    echo -e "${GREEN}🚀 neoxagent All-in-One Installer v0.5${NC}"
    echo "═══════════════════════════════════════════"
    if [ -n "$DOMAIN" ]; then
...
log_info "Compiling hev-socks5-tproxy..."

git clone --depth 1 --recursive https://github.com/heiher/hev-socks5-tproxy.git "$HEV_TPROXY_DIR" 2>/dev/null
cd "$HEV_TPROXY_DIR"
