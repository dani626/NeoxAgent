...
async fn main() {
    ...
    let podman = if socket_uri.contains("://") {
        Podman::new(socket_uri).unwrap_or_else(|e| {
            tracing::error!("Failed to create Podman client: {}", e);
            std::process::exit(1);
        })
    } else {
    ...
    match podman.info().await {
        Ok(info) => {
            let version = info.version.as_ref()
                .and_then(|v| v.version.as_deref()).unwrap_or("unknown");
            let containers = info.store.as_ref()
                .and_then(|s| s.container_store.as_ref())
                .and_then(|cs| cs.number).unwrap_or(0);
            let rootless = info.host.as_ref()
                .and_then(|h| h.security.as_ref())
                .and_then(|s| s.rootless).unwrap_or(false);
            tracing::info!("🐙 Podman v{} — {} containers — rootless: {}",
                version, containers, rootless);
        }
        Err(e) => {
    ...
    let app = Router::new()
        ...
        .route("/api/containers/{id}/logs",    get(routes::containers::get_container_logs))
        .route("/api/containers/{id}/rename",  post(routes::containers::rename_container))
        .route("/api/containers/{id}/start",   post(routes::containers::start_container))
        ...
        .route(
            "/api/pods/{id}",
            get(routes::pods::get_pod).delete(routes::pods::delete_pod),
        )
        .route("/api/pods/{id}/logs",       get(routes::pods::get_pod_logs))
        .route("/api/pods/{id}/rename",     post(routes::pods::rename_pod))
        .route("/api/pods/{id}/proxy",      post(routes::pods::update_proxy))
        .route("/api/pods/{id}/start",      post(routes::pods::start_pod))
        .route("/api/pods/{id}/stop",       post(routes::pods::stop_pod))
        .route("/api/pods/{id}/restart",    post(routes::pods::restart_pod))
        .route("/api/pods/{id}/kube",       get(routes::pods::generate_kube_yaml))
        .route(
        ...
        .route(
            "/api/pods/{id}/files",
            get(routes::files::list_files).delete(routes::files::delete_file),
        )
        .route(
            "/api/pods/{id}/files/content",
            get(routes::files::read_file).put(routes::files::write_file),
        )
        .route("/api/pods/{id}/files/create-dir", post(routes::files::create_directory))
        .route("/api/pods/{id}/files/rename",     post(routes::files::rename_file))
        .route("/api/pods/{id}/files/upload",     post(routes::files::upload_file))
        .route("/api/pods/{id}/files/download",   get(routes::files::download_file))
        // ─── Backups ──────────────────────────────────────────────────
        .route(
            "/api/pods/{id}/backups",
            get(routes::backups::list_backups).post(routes::backups::create_backup),
        )
        .route(
            "/api/pods/{id}/backups/{backup_id}",
            get(routes::backups::get_backup_info).delete(routes::backups::delete_backup),
        )
        .route("/api/pods/{id}/backups/{backup_id}/download", get(routes::backups::download_backup))
        .route("/api/pods/{id}/backups/{backup_id}/restore",  post(routes::backups::restore_backup))
        // ─── Images ────────────────────────────────────────────────────
        .route("/api/images",             get(routes::images::list_images))
        .route("/api/images/pull",         post(routes::images::pull_image))
        .route("/api/images/search",       get(routes::images::search_images))
        .route("/api/images/pull/stream",  get(routes::images::pull_image_stream))
        .route("/api/images/{id}",         delete(routes::images::delete_image))
        .route("/api/images/{id}/inspect", get(routes::images::inspect_image))
        // ─── Systemd ─────────────────────────────────────────────────
        .route("/api/pods/{id}/systemd/generate", post(routes::systemd::generate_systemd))
        .route("/api/pods/{id}/systemd/enable",   post(routes::systemd::enable_systemd))
        .route("/api/pods/{id}/systemd/disable",  post(routes::systemd::disable_systemd))
        .route("/api/pods/{id}/systemd/status",   get(routes::systemd::systemd_status))
        // ─── Middleware ──────────────────────────────────────────────
    ...
    if state.config.tls.enabled {
        ...
        if cert_path.is_empty() || key_path.is_empty() {
            tracing::error!("❌ TLS enabled but cert_path or key_path is empty");
            std::process::exit(1);
        }
        ...
        let addr: std::net::SocketAddr = bind_addr.parse().unwrap_or_else(|e| {
            tracing::error!("Bad bind address: {}", e);
            std::process::exit(1);
        });
